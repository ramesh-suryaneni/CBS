...
public class ContractorServiceImplTest {
	...
	public void geContractorEmployeeDetailsIfExistsInDB() {
		...
		Page<ContractorEmployeeSearchDto> contractorEmployeeDto = contractorServiceImpl.geContractorEmployeeDetails(0, 5,"roleId","ASC");
	...
	public void geContractorEmployeeDetailsByRoleIdIfRoleIdExistsInDB() {
		...
		when(contractorEmployeeSearchRepository.findByRoleId(Mockito.anyLong(), Mockito.any())).thenReturn(createContractorEmployeePagedData());
		...
		verify(contractorEmployeeSearchRepository, times(1)).findByRoleId(1000L, PageRequest.of(0, 5, Sort.by(Direction.ASC,"roleId")));
		String role = contractorEmployeeDto.getContent().get(0).getRole();
		assertEquals("2D", role);
	}
	...
	public void geContractorEmployeeDetailsByRoleIdIfRoleIdNotExistsInDB() {
		...
		when(contractorEmployeeSearchRepository.findByRoleId(Mockito.anyLong(), Mockito.any())).thenReturn(contractorEmployeeSearch);
		...
		verify(contractorEmployeeSearchRepository, times(1)).findByRoleId(1001L, PageRequest.of(0, 5, Sort.by(Direction.DESC,"roleId")));
	...
	public void geContractorEmployeeDetailsByRoleIdAndNameIfNameExistsInDB() {
		...
		when(contractorEmployeeSearchRepository.findByRoleIdAndContractorEmployeeNameContains(Mockito.anyLong(), Mockito.anyString(), Mockito.any()))
		.thenReturn(createContractorEmployeeSearchPagedDataForName());
		...
		verify(contractorEmployeeSearchRepository, times(1)).findByRoleIdAndContractorEmployeeNameContains(1000L, "Alex", PageRequest.of(0, 5, Sort.by(Direction.ASC,"dayRate")));
	...
	public void geContractorEmployeeDetailsByRoleIdAndNameIfNameNotExistsInDB() {
		...
		verify(contractorEmployeeSearchRepository, times(1)).findByRoleIdAndContractorEmployeeNameContains(1000L,"Test", PageRequest.of(0, 5, Sort.by(Direction.ASC,"dayRate")));
		assertTrue(contractorEmployeeDto.getContent().isEmpty());
	}
	...
	public void getContractorEmployeeDetailsIfNotPresentInDB() {
		...
		when(contractorRepository.findAll(PageRequest.of(0, 5, Sort.by(Direction.DESC, "contractorName")))).thenReturn(contractorPage);
	...
	public void getContractorDeatilsContainingNameIfNameExistsInDB() {
		...
		when(contractorRepository.findByContractorNameContains("Im",PageRequest.of(0, 5, Sort.by(Direction.ASC, "contractorName"))))
		.thenReturn(createContractorPagedData());
		
		Page<ContractorDto> contractorDto = contractorServiceImpl.getContractorDeatilsContainingName("Im",0, 5,"contractorName","ASC");
		verify(contractorRepository, times(1)).findByContractorNameContains("Im",PageRequest.of(0, 5, Sort.by(Direction.ASC,"contractorName")));
		String contractorName = contractorDto.getContent().get(0).getContractorName();
		assertEquals("Imagination", contractorName);
	}
	...
	public void getContractorDeatilsContainingNameIfNameNotExistsInDB() {
		...
		Page<Contractor> contractorPage = new PageImpl<>(new ArrayList<Contractor>(),PageRequest.of(0,5),0); 	
		when(contractorRepository.findByContractorNameContains("Test", PageRequest.of(0, 5, Sort.by(Direction.DESC, "contractorName")))).thenReturn(contractorPage);
		
		Page<ContractorDto> contractorDto = contractorServiceImpl.getContractorDeatilsContainingName("Test",0, 5,"contractorName","DESC");
		verify(contractorRepository, times(1)).findByContractorNameContains("Test", PageRequest.of(0, 5, Sort.by(Direction.DESC,"contractorName")));
		assertTrue(contractorDto.getContent().isEmpty());
	}
	
	@Test
	public void getContractorByContractorIdIfIdExistInDB() {
		Contractor contractor = Mockito.mock(Contractor.class);//new Contractor();
	...
	public void getContractorEmployeeByContractorIdAndEmployeeIdIfExistInDB() {
		...
		contractorEmployeeDto.setEmployeeName("John");
		contractorEmployeeDto.setKnownAs("Alias");
		
		when(contractorEmployeeRepository.findContractorEmployeeByContractorIdAndEmployeeId(Mockito.anyLong(), Mockito.anyLong()))
		.thenReturn(contractorEmployee);
		when(contractorEmployeeMapper.toContractorEmployeeDtoFromContractorEmployee(contractorEmployee)).thenReturn(contractorEmployeeDto);
		
		ContractorEmployeeDto response = contractorServiceImpl.getContractorEmployeeByContractorIdAndEmployeeId(2001L, 3001L);
	...
	public void getContractorEmployeeByContractorIdAndEmployeeIdIfNotExistInDB() {
		...
		when(contractorEmployeeRepository.findContractorEmployeeByContractorIdAndEmployeeId(Mockito.anyLong(), Mockito.anyLong()))
		.thenReturn(contractorEmployee);
		when(contractorEmployeeMapper.toContractorEmployeeDtoFromContractorEmployee(contractorEmployee)).thenReturn(null);
	...
	public void addContractorEmployeeSucess() {
		...
		when(contractorEmployeeMapper.toContractorEmployeeDtoFromContractorEmployee(Mockito.any())).thenReturn(contractorEmployeeDto);
		
		ContractorEmployeeDto response = contractorServiceImpl.addContractorEmployee(2001L, contractorEmployeeRequest);
		
		assertEquals("Alex", response.getEmployeeName());
